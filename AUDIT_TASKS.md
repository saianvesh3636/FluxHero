...
- [ ] Replace bare `except Exception: pass` in `backend/storage/sqlite_store.py:243` `_write_worker` with `except sqlite3.Error`, log and handle explicitly
...
- [ ] Add Python logging configuration module at `backend/core/logging_config.py` using standard library logging
...
- [ ] Create authentication middleware at `backend/api/auth.py` with token validation function
- [ ] Update WebSocket endpoint in `backend/api/server.py:587-591` to validate token before `websocket.accept()`
- [ ] Add configuration for auth secret key (prepare for Phase 4 centralized config)
- [ ] Add tests for WebSocket authentication (valid token, invalid token, missing token)
