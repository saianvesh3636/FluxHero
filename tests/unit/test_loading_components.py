...
def test_loading_spinner_variants():
    ...
    assert "'spinner' | 'dots' | 'pulse' | 'skeleton'" in content, (
        "LoadingVariant type not properly defined"
    )
...
def test_live_page_uses_loading_spinner():
    ...
    assert "Loading live data" in content or "loading" in content.lower(), (
...
def test_analytics_page_uses_loading_components():
    ...
    assert (
        "LoadingOverlay" in content or "SkeletonCard" in content or "LoadingSpinner" in content
...
def test_history_page_uses_loading_components():
    ...
    # Check for import
    assert "from '../components/LoadingSpinner'" in content, "LoadingSpinner not imported"
    assert "SkeletonTable" in content or "LoadingSpinner" in content, (
        "Loading components not imported"
    )
    ...
    for page_path in pages:
        ...
        if "setLoading(true)" in content or "setIsRunning(true)" in content:
            assert "finally" in content, (
                f"{page_path}: Loading state set but no finally block found"
            )
...
def test_skeleton_loaders_have_proper_structure():
    ...
    skeleton_card_match = re.search(
        r"export const SkeletonCard.*?return.*?</div>", content, re.DOTALL
    )
    assert skeleton_card_match, "SkeletonCard structure not found"

    # Check for SkeletonTable structure
    skeleton_table_match = re.search(
        r"export const SkeletonTable.*?return.*?</div>", content, re.DOTALL
    )
