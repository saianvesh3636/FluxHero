...
class TestBacktestPage(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.backtest_page_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "", "frontend", "app", "backtest", "page.tsx"
        )
    ...
    def test_backtest_page_is_client_component(self):
        ...
        self.assertIn("'use client'", content, "Backtest page should be a client component")
    ...
    def test_backtest_config_interface_defined(self):
        ...
        for field in required_fields:
            self.assertIn(f"{field}:", content, f"BacktestConfig should include {field} field")
    ...
    def test_backtest_result_interface_defined(self):
        ...
        for metric in required_metrics:
            self.assertIn(f"{metric}:", content, f"BacktestResult should include {metric} field")
    ...
    def test_symbol_selector_present(self):
        ...
        for symbol in symbols:
            self.assertIn(symbol, content, f"Symbol selector should include {symbol} option")
    ...
    def test_strategy_parameter_sliders_present(self):
        ...
        for param in parameters:
            self.assertIn(param, content, f"Strategy parameters should include {param}")
        ...
        for config in slider_configs:
            self.assertIn(config, content, f"Strategy slider {config} should be present")
    ...
    def test_performance_metrics_display(self):
        ...
        for metric in metrics:
            self.assertIn(f"results.{metric}", content, f"Results should display {metric}")
    ...
    def test_success_criteria_validation_display(self):
        ...
        self.assertTrue(
            "Sharpe" in content and "0.8" in content, "Should display Sharpe ratio criteria"
        )
        self.assertTrue(
            "DD" in content or "Drawdown" in content, "Should display drawdown criteria"
        )
        self.assertTrue(
            "Win Rate" in content and "45" in content, "Should display win rate criteria"
        )
    ...
    def test_trade_log_table_present(self):
        ...
        for column in columns:
            self.assertIn(column, content, f"Trade log should include {column} column")
    ...
    def test_default_config_values_set(self):
        ...
        for default in defaults:
            self.assertIn(default, content, f"Default value {default} should be set")
    ...
    def test_responsive_design_classes(self):
        ...
        for cls in responsive_classes:
            self.assertIn(cls, content, f"Responsive class {cls} should be present")
    ...
    def test_accessibility_labels(self):
        ...
        self.assertGreater(label_count, 10, "Form should have multiple labels for accessibility")
    ...
    def test_file_structure_valid_typescript(self):
        ...
        self.assertTrue("boolean" in content, "File should use boolean type")
    ...
    def test_all_sections_present(self):
        ...
        for section in sections:
            self.assertIn(section, content, f"Page should include '{section}' section")
...
class TestBacktestPageIntegration(unittest.TestCase):
    ...
    def test_state_management_complete(self):
        ...
        for state in states:
            self.assertIn(f"{state},", content, f"State variable '{state}' should be managed")
