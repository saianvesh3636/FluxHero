...
class TestDarkModeImplementation:
    ...
    def test_theme_context_has_provider(self):
        ...
        with open(get_project_path("frontend/utils/theme-context.tsx")) as f:
            content = f.read()
    ...
    def test_theme_context_has_storage_key(self):
        ...
        with open(get_project_path("frontend/utils/theme-context.tsx")) as f:
            content = f.read()
            assert "STORAGE_KEY" in content or "fluxhero-theme" in content, (
                "Storage key not defined"
            )

    def test_theme_toggle_imports_hook(self):
        """Test that theme toggle component imports useTheme hook."""
        with open(get_project_path("frontend/components/ThemeToggle.tsx")) as f:
            content = f.read()
    ...
    def test_layout_includes_theme_provider(self):
        ...
        with open(get_project_path("frontend/app/layout.tsx")) as f:
            content = f.read()
            assert "ThemeProvider" in content, "ThemeProvider not included in layout"
            assert "from '../utils/theme-context'" in content, (
                "Theme context not imported in layout"
            )
    ...
    def test_layout_includes_theme_toggle(self):
        ...
        with open(get_project_path("frontend/app/layout.tsx")) as f:
            ...
            assert "from '../components/ThemeToggle'" in content, (
                "ThemeToggle not imported in layout"
            )

    def test_globals_css_has_css_variables(self):
        """Test that globals.css defines CSS variables for theming."""
        with open(get_project_path("frontend/app/globals.css")) as f:
            content = f.read()
    ...
    def test_theme_context_has_mounted_state(self):
        ...
        with open(get_project_path("frontend/utils/theme-context.tsx")) as f:
            content = f.read()
            assert "mounted" in content or "Mounted" in content, (
                "Mounted state not found (prevents hydration issues)"
            )
    ...
    def test_body_uses_css_variables(self):
        ...
        with open(get_project_path("frontend/app/globals.css")) as f:
            ...
            assert (
                "color: var(--color-text-primary)" in content
                or "background: var(--color-bg-primary)" in content
            ), "Body should use CSS variables for theming"
    ...
    def test_css_transition_smooth(self):
        ...
        with open(get_project_path("frontend/app/globals.css")) as f:
            ...
            assert "transition" in content, (
                "CSS should include transitions for smooth theme changes"
            )
    ...
    def test_theme_types_defined(self):
        ...
        with open(get_project_path("frontend/utils/theme-context.tsx")) as f:
            ...
            assert "'light'" in content and "'dark'" in content, (
                "Light and dark theme values not defined"
            )
    ...
    def test_theme_context_error_handling(self):
        ...
        with open(get_project_path("frontend/utils/theme-context.tsx")) as f:
            content = f.read()
            assert "throw new Error" in content or "throw Error" in content, (
    ...
    def test_theme_toggle_position_fixed(self):
        ...
        with open(get_project_path("frontend/app/globals.css")) as f:
            content = f.read()
            toggle_container_section = (
                content[content.find(".theme-toggle-container") :]
                if ".theme-toggle-container" in content
                else ""
            )
            assert (
                "position: fixed" in toggle_container_section
                or "position:fixed" in toggle_container_section
            ), "Theme toggle container should be fixed position"
    ...
    def test_theme_applies_to_document(self):
        ...
        with open(get_project_path("frontend/utils/theme-context.tsx")) as f:
            content = f.read()
            assert "document.documentElement" in content or "document.body" in content, (
                "Theme should be applied to document"
            )
            assert "setAttribute" in content or "classList" in content, (
                "Theme should be set via attribute or class"
            )
