...
def test_compression_effectiveness(parquet_store):
    ...
    assert size is not None
    assert size < 200000, (
        f"Compressed size {size} bytes should be reasonable (<200KB) for 1000 candles"
    )
...
def test_save_candles_with_indicators_logging(
    parquet_store, sample_candle_data_with_indicators, caplog
):
...
def test_clear_all_cache_logging(
    parquet_store, sample_candle_data, sample_candle_data_with_indicators, caplog
):
