    ...
    if signal_explanation is None:
        signal_explanation = {
            ...
            "formatted_reason": (
                ...
                f"Reason: Volatility (ATR=3.2, Normal) + {signal_reason}\n"
                f"Regime: STRONG_TREND (ADX=32.0, R²=0.81)\n"
                f"Risk: $1,000 (1.00% account), Stop: $415.00"
            ),
            "compact_reason": (
                f"BUY @ ${entry_price:.2f} | {signal_reason} | "
                f"STRONG_TREND (ATR=3.2, Normal) | Risk: $1,000 (1.00%)"
            ),
        }
...
class TestFilteringFunctionality:
    ...
    def test_filter_by_date_range(self):
        ...
        filtered_trades = [t for t in trades if start_time <= t["entry_time"] <= end_time]
    ...
    def test_search_by_signal_reason(self):
        ...
        kama_trades = [t for t in trades if "KAMA" in t["signal_reason"].upper()]
    ...
    def test_multiple_filters_combined(self):
        ...
        filtered = [t for t in trades if t["symbol"] == "SPY" and t["strategy"] == "TREND"]
...
class TestCSVExport:
    ...
    def test_export_with_commas_in_text(self):
        ...
        trade = create_mock_trade(
            1, signal_reason="KAMA crossover, high volume", signal_explanation=signal_explanation
        )
